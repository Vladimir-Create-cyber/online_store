	...
	'add': function(product_id, quantity) {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']['stock']) {
					$('.alerts').append($('<div class="alert alert-danger"><span class="alert__text"> ' + json['error']['stock'] + ' </span><a href="#" class="alert__close">x</a></div>').hide().fadeIn(500));
						alertClose();
				}
...
function spinner() {
	...
	// krumax dev
	// категория table
	$( ".catalogue__table-spinner .spinner-wrap a" ).on( "click", function( event, ui ) { 
		
		var maximum = parseInt($(this).prev(".spinner").attr("maximum"));
		var value = parseInt($(this).prev(".spinner").val());
		if (value > maximum) $(this).prev(".spinner").val(maximum);
		//alert(maximum);
		//$('#cartcontent').submit();
	...
	// категория list
	$( ".products-full-list__spinner .spinner-wrap span a" ).on( "click", function( event, ui ) { 
		var maximum = parseInt($(this).prev(".spinner").attr("maximum"));
		var value = parseInt($(this).prev(".spinner").val());
		if (value > maximum) $(this).prev(".spinner").val(maximum);
		// alert(maximum);
	...
	// категория popup
	$( ".products-qty-info-spinner .spinner-wrap span a" ).on( "click", function( event, ui ) { 
		var maximum = parseInt($(this).prev(".spinner").attr("maximum"));
		var value = parseInt($(this).prev(".spinner").val());
		if (value > maximum) $(this).prev(".spinner").val(maximum);
		// alert(maximum);
...
function other() {
	...
	$('body').on('click','input[name=\'multiselect_0\']', function(e) {
		...
		var currentLang = currentPathArray[1];
		...
		if (currentPathArray.length == 1) {
			...
			if (currentLang == 'ch') {
				...
				var updatedPath = currentPathArray.join('/');
			}
			if (currentLang == 'ru') {
				currentPathArray.splice(1,1);
				var updatedPath = currentPathArray.join('/');
			...
			if (newLang == 'ru-ru') {
				updatedPath.replace('/' + currentLang + '/', '/ru/')
			}
			if (newLang == 'en-gb') {
				updatedPath.replace('/' + currentLang + '/', '')
			}
			if (newLang == 'pl-pl') {
				updatedPath.replace('/' + currentLang + '/', '/pl/')
			}
			if (newLang == 'es-es') {
				updatedPath.replace('/' + currentLang + '/', '/es/')
			}
			if (newLang == 'zh-CN') {
				updatedPath.replace('/' + currentLang + '/', '/ch/')
			}
		}
...
function gtag_report_conversion(url) {
  ...
  gtag('event', 'conversion', {
      'send_to': 'AW-930963822/3UhPCL_H5e0BEO7C9bsD',
