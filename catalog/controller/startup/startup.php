...
class ControllerStartupStartup extends Controller {
	public function index() {
		...
		$kd_lang = array('ru', 'pl', 'en-gb');
		foreach($language_from_url as $lang){
			if(in_array($lang, $kd_lang)){
		...
		// if ($language_from_url && in_array($language_from_url, $kd_lang)){

		// switch ($language_from_url) {
		// 			case 'pl':
		// 				$code = 'pl-pl';
		// 				break;
        ...
        foreach ($query->rows as $result) {
            $language_path = strtolower($result['name']);

            if(str_replace('/'.$language_path.'/','',$_SERVER['REQUEST_URI'])!=$_SERVER['REQUEST_URI'])
            {
                $language = new Language($result['code']);
                $language->load($result['code']);
                $this->registry->set('language', $language);
