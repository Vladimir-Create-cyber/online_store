...
class ModelXlsPricelistHelperModels extends Model {
	...
	public function getCustomerGroup1($customer_group_id, $language_id) {
		...
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_group cg LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (cg.customer_group_id = cgd.customer_group_id) WHERE cg.customer_group_id = '" . (int)$customer_group_id . "'"); /// на самом деле разделение групп на языки до конца не реализовано в CMS. Так что опускаем этот момент.
		return $query->row;
	}
